...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      // Cross-tab security: if a different user signs in or any user signs out
      // in another tab, force a hard reload to clear all in-memory React state.
      if (event === "SIGNED_OUT") {
        window.location.href = "/login";
        return;
      }
      if (event === "SIGNED_IN" && u) {
        // If the currently loaded user differs from the new session user, hard reload
        // to prevent stale data from the previous user being visible.
        setUser((prev) => {
          if (prev && prev.id !== u.id) {
            window.location.reload();
            return prev; // reload in flight — keep current value
          }
          return u;
        });
        setRole(resolveRole(u));
